...
function validReport(path, plr)
	if (path and plr) then
		dbQuery(adminDB, "UPDATE reports SET status=? WHERE reportImagePath = ?", "Valid",  path)
...
function inValidReport(path, plr)
	if (path and plr) then
		dbQuery(adminDB, "UPDATE reports SET status=? WHERE reportImagePath = ?", "Invalid",  path)
