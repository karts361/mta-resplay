...
function setPlayerDetails(plr, IP, account, aclGroup, serial)
	...
	if (isElementFrozen(plr)) then
		guiSetText(adminButton[4], "Разморозить")
	else
		guiSetText(adminButton[4], "Заморозить")
	end
	guiSetText(adminLabel[1], "Имя : "..getPlayerName(plr))
	guiSetText(adminLabel[2], "IP : "..IP)
	guiSetText(adminLabel[3], "С/Н : "..serial)
	guiSetText(adminLabel[4], "Аккаунт : "..account)
	guiSetText(adminLabel[5], "ACL Группа : "..aclGroup)
	guiSetText(adminLabel[6], "ХП : "..getElementHealth(plr))
	guiSetText(adminLabel[7], "Броня : "..getPedArmor(plr))
	guiSetText(adminLabel[8], "Скин : "..getElementModel(plr))
	if (getPlayerTeam(plr))then
		guiSetText(adminLabel[9], "Статус : "..getTeamName(getPlayerTeam(plr)))
	else
		guiSetText(adminLabel[9], "Статус : None")
	end
	guiSetText(adminLabel[10], "Профессия : "..(getElementData(plr, "Occupation") or "None"))
	guiSetText(adminLabel[11], "Оружие : "..weapon)
	guiSetText(adminLabel[12], "Пинг : "..getPlayerPing(plr))
	guiSetText(adminLabel[13], "Местоположение : "..getZoneName(x, y, z).." ("..getZoneName(x, y, z, true)..")")
	guiSetText(adminLabel[14], "X : "..x)
	guiSetText(adminLabel[15], "Y : "..y)
	guiSetText(adminLabel[16], "Z : "..z)
	guiSetText(adminLabel[17], "Измерение : "..getElementDimension(plr))
	guiSetText(adminLabel[18], "Интерьер : "..getElementInterior(plr))
	guiSetText(adminLabel[19], "Транспортное средство : "..veh)
	guiSetText(adminLabel[20], "ХП ТС : "..vehHealth)
	guiSetText(adminLabel[21], "Группа : "..(getElementData(plr, "groupName") or "None").." ("..(getElementData(plr, "groupRank") or "None")..")")
	guiSetText(adminLabel[22], "Аккаунт создан : "..(getElementData(plr, "accountCreation") or "N/A"))
	guiSetText(adminLabel[23], "Игровое время : "..(getElementData(plr, "PlayTime") or "None"))
	guiSetText(adminLabel[24], "Деньги : "..getPlayerMoney(plr))
end
