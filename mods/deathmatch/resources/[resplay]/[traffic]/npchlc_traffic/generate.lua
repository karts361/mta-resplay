function initTrafficGenerator()
	traffic_density = {peds = 0,cars = 0.003,boats = 0.0,planes = 0.0}
...
function spawnTrafficInSquare(x,y,dim,trtype)
	...
	while count_needed >= 1 do
		...
		elseif create and trtype == "cars" then
			...
			local ex, ey
			local elems = getElementsByType("vehicle")
			local carsToCheck = 0
			for _,elem in ipairs(elems) do
				ex, ey = getElementPosition( elem )
				if( getDistanceBetweenPoints2D( x, y, ex, ey ) < 150.0 ) and ( getElementDimension(elem) == dim ) then
					carsToCheck = carsToCheck + 1
				end
			end
			if(carsToCheck < 10) then
			
			elems = getElementsByType("player")
			local plrsToCheck = 0
			for _,elem in ipairs(elems) do
				ex, ey = getElementPosition( elem )
				if( getDistanceBetweenPoints2D( x, y, ex, ey ) < 50.0 ) and ( getElementDimension(elem) == dim ) then
					plrsToCheck = plrsToCheck + 1
					break
				end
			end
			
			if(plrsToCheck == 0) then
