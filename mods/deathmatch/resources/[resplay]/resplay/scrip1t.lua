...
introCameras = {
	{ 1760.8, -1943.0, 17.6, 1787.3, -1926.1, 13.4 },
...
function receiveFriendRequest()
	...
	if not requestFound then
		...
		friendsUpdateRequestList()
		msgAdd(string.format("%s хочет добавить вас в друзья. Нажмите клавишу '5', чтобы попасть в меню друзей.", friendName))
	end
...
function msgDraw()
	if(curMsg == nil) then
		...
		if(curMsg[2] > 9875) then
			...
			startY = 0--sH-80*scaleMult
			curW = 25+260*progress*scaleMult
			curH = 25
			isTextDrawing = false
		...
		elseif(curMsg[2] > 9750) then
			...
			startY = 0--sH-(80+60*progress)*scaleMult
			curW = 25+260*scaleMult
			curH = 25+120*progress*scaleMult
			isTextDrawing = false
		
		elseif(curMsg[2] > 250) then
			startX = sW/2-5-130*scaleMult
			startY = 0--sH-140*scaleMult
			curW = 25+260*scaleMult
			curH = 25+120*scaleMult
			isTextDrawing = true
...
function startIntro()
	...
	bindKey("3", "down", "Inventory")
	bindKey("4", "down", "Actions menu")
	bindKey("5", "down", "Friends menu")
	bindKey("6", "down", "Show respect")
	...
	-- DEBUG MODE OPTIONS
	debugGuiStr = guiCreateLabel(5, sH-15, sW-5, 15, "www.resplay.ru | vk.com/resplay | discord.gg/m6tsZ7x", false)
...
function answerOpen(qnum, plrName, questionText)
	if not isElement(answerWin) then
		...
		guiSetProperty(answerMemo, "MaxTextLength", "64")
...
function playerTeamChat(timeStr, grp, msg, clrr, clrg, clrb)
	...
	elseif playerGroups[grp] then
		outputChatBox(timeStr.."["..playerGroups[grp][1].."] "..getPlayerName(source)..": #FFFFFF"..msg, playerGroups[grp][2], playerGroups[grp][3], playerGroups[grp][4], true)
