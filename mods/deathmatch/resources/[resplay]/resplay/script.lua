...
function initialize_skin_frac()
	local window2 = guiCreatewindow2(sWidth-340, (sHeight-500)/2, 340, 500, "Выбор скина", false)
    skin_list_gui2 = guiCreateGridList(10, 25, 320, 425, false, window2)
	col_name2 = guiGridListAddColumn(skin_list_gui2, "ID", 0.25)
	--col_cost = guiGridListAddColumn(skin_list_gui2, "Цена", 0.25)
	btn_buy2 = guiCreateButton(10, 456, 158, 40, "Выбрать", false, window2)
	btn_close22 = guiCreateButton(172, 456, 158, 40, "Закрыть", false, window2)
	guiSetVisible(window2, false)
	addEventHandler("onClientGUIClick", btn_close2, function( )
		guiSetVisible(window2, false)
		showCursor(false)
		setElementModel(localPlayer, model2)
		setElementFrozen(localPlayer, false)
	end, false)
...
function showSkinFrac()
	...
	setTimer(setElementRotation, 1000, 1, localPlayer, rx,ry, rz-180)
	setTimer(fadeCamera, 1200, 1, true)
	model2 = getElementModel(localPlayer)
end
addEvent("showSkinFrac", true)
...
function selectSkin2()
	...
	if (not id) and (not cost) then return end
	if id == model2 then
	msgAdd("Вы уже одели эту одежду!")
	return
	end
	setElementModel(localPlayer, model2)
	setElementFrozen(localPlayer, false)
