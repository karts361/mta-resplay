...
function closePanelOnLeave(hitElement)
	--outputDebugString("CINEMA: onClientColShapeLeave")
	
	if hitElement == localPlayer then
		--outputDebugString("CINEMA: hitElement")
		cleanUp()
...
function createPlaylist()
	playlist = dxGrid:Create(852, 298, 391, 650)
	videos = playlist:AddColumn("Плейлист", 391)
end

function createUserlist()
	userlist = dxGrid:Create(852, 298, 391, 650)
	names = userlist:AddColumn("Посетители", 391)
end
...
function showPlaylist()
	if not isTimer(showDelay) then
		...
		outputChatBox("Пожалуйста, подождите еще несколько секунд, прежде чем использовать эту команду",245,180,140,true)
	end
end
...
function syncPlaylist(list)
	if playlist then
		...
		for i,v in pairs(list) do			
			if i == 2 then
				playlist:AddItem(videos,"Следующее:","",148,227,150)
			end
			if i == 1 then
				playlist:AddItem(videos,"Сейчас играет:","",148,227,150)
...
function showList(theList)
	...
	if theList == "UserList" then
		if userlist then
			...
			plTabColor = {18, 20, 20, 255}
			ulTabColor = {51, 51, 51, 255}
			activeList = "UserList"
...
function syncVotes(voteCount,maxVotes)
	if voteCount then
		...
		function displayVotes()
			if isPanelVisible then
				Text("Votes to skip: "..voteCount.."/"..maxVotes, 1051, 207, 1192, 233, tocolor(255, 255, 255, 255), fontScaleSmall, fontType, "center", "center", true, false, true, false, false)
			end
		end
	...
	if isElementWithinColShape(localPlayer,cinemaCol) then
		...
		if title and tonumber(length) then
			...
			function displayTitle()
				...
				if isPanelVisible then
					Text("Сейчас играет: "..title.."\nДлина: "..remaining.." / "..length.."\nДобавил: "..nick, 852, 170, 1243, 249, tocolor(255, 255, 255, 255), fontScaleSmall, fontType, "left", "top", true, false, true, false, false)
					
					Rectangle(852, 239, 391, 10, tocolor(217, 217, 217, 255), true)
					Rectangle(852, 239, width, 10, tocolor(51, 51, 51, 255), true)
					...
					Text("Skip", 1197, 207, 1243, 233, tocolor(255, 255, 255, 255), fontScaleSmall, fontType, "center", "center", true, false, true, false, false)
...
function cleanUp()
	--outputDebugString("CINEMA: cleanUp()")
	setBrowserVolume(Screen,0)
...
function changeScreenVolume(state)
	...
	if state == "up" then
		screenVolume = math.round(screenVolume + 0.1, 2)
