...
if not fileExists("cfg/settings.xml") then
    ...
    xmlSaveFile(node)
    xmlUnloadFile(node)
    --outputDebugString("RESPLAY settings created Successfully.")
end

function changeSetting(settingName)
    if getElementType(source) =="player" and not isPlayerManager(source) then
        triggerClientEvent(source,"onDxPopupMessage",resourceRoot,"У вас нет разрешения для этого.", 3500, 255, 255, 255, 255, 0, 0)
        return
    end
		...
		if not isPedInVehicle(plr) then
			if source == entryMarker1 or source == entryMarker2 then
				if isOpen or isObjectInACLGroup("user."..getAccountName(getPlayerAccount(plr)),aclGetGroup(aclGroup)) or isPlayerManager(plr) then 
					...
					triggerClientEvent(plr,"onDxPopupMessage",resourceRoot, "Простите, но мы закрыты. Пожалуйста, приходите позже.",4000,255,0,0,255,255,255,true)
				end
        ...
        if getElementType(source) == "player" then
            if isPlayerManager(source) then
                if selectedPlayer then
                    ...
                    triggerClientEvent(source,"onDxPopupMessage",resourceRoot,"Вы были кикнуты "..getPlayerName(selectedPlayer).." из театра.")
                ...
                triggerClientEvent(plr,"onDxPopupMessage",resourceRoot,"Вас выгнал из театра "..getPlayerName(source),6000,255,255,255,255,0,0,true)
            else
                triggerClientEvent(source,"onDxPopupMessage",resourceRoot,"У вас нет разрешения для этого.",3500,255,255,255,255,0,0)
            end
        end
